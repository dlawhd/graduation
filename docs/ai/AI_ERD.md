...
- `slot_size_ratio`는 실제 이미지 너비 비율이 아니라 슬라이더 위치이므로, 나중에 크기 변환 공식을 바꾸면 기존 Jar의 Slot도 변할 수 있다. **기존 Jar 렌더링 공식은 유지**하고 새 정책이 필요하면 버전 구분 방법을 별도 논의한다. 예시 10~40%를 검증 없이 불변 사양이라고 선언하지 않는다.
