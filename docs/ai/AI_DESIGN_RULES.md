...
23번 Slot Editor에서 다음 변환 규칙을 사용한다.

- 실제 너비 = 이미지 너비 × `(0.12 + 0.16 × slot_size_ratio)`
- 실제 높이 = 슬롯 너비 ÷ `3.5` (가로:세로 = 3.5:1)
- 슬라이더 0 / 0.5 / 1은 각각 이미지 너비의 12% / 20% / 28%다.
- 원본·PIXEL 480×480과 일반 AI 1024×1024 모두 같은 비율을 사용한다.
- 좌표는 소수 다섯 자리로 저장하며, 반올림 후에도 슬롯 전체가 이미지 안에 있도록 보정한다.

React의 `slotGeometry.mjs`와 `JarSlotOverlay`를 이후 최종 미리보기·Jar 표시에서도 재사용한다.
서버는 `JarSlotGeometry`로 저장 및 Finalize 전에 동일한 경계 규칙을 검사한다.
이미 저장한 Jar의 표시가 달라지므로 이 공식은 기존 데이터에 대해 임의 변경하지 않는다.
